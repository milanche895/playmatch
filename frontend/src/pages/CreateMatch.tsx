...
export default function CreateMatch() {
  ...
  function generateAvailableTimeSlots(
    ...
    for (let d = new Date(now); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      const startHour = dayHours.start
        ? parseInt(dayHours.start.split(":")[0])
        : 9;
      ...
      for (let hour = startHour; hour < endHour; hour++) {
        ...
        const dateOnly = `${slotDate.getFullYear()}-${String(
          slotDate.getMonth() + 1
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!hasCookie) {
      console.warn(
        "Warning: No token cookie found. You may need to log in again."
      );
    }
        ...
        <Stack spacing={2} maxWidth={480}>
          ...
          <Stack direction="row" spacing={1}>
            <TextField
              ...
              helperText={
                fields.length === 0
                  ? "Nema dostupnih terena. Dodajte jedan prvo."
                  : ""
              }
            >
              {fields.map((f) => (
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <Stack spacing={1}>
                      ...
                      {selectedDate ? (
                        (() => {
                          const dateSlots = availableTimeSlots.filter(
                    ...
                    <Alert severity="warning">
                      <Typography variant="body2">
                        ...
                        {(() => {
                          ...
                          const deadlineHours =
                            selectedField.registrationDeadlineHours ?? 24;
                          const deadline = new Date(matchDate);
                          deadline.setHours(
                            deadline.getHours() - deadlineHours
                          );
                        ...
                        ({selectedField.registrationDeadlineHours ?? 24} sati
                        pre meča)
                      </Typography>
                    </Alert>
                  </Stack>
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              if (
                value === "" ||
                (!isNaN(value as number) && (value as number) >= minPlayers)
              ) {
                setMaxPlayers(value);
              }
            }}
            helperText={
              maxPlayers === ""
                ? "Ostavite prazno ako nema maksimuma"
                : `Maksimum: ${maxPlayers} igrača`
            }
            fullWidth
          />
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Box>
              <Typography variant="body2" sx={{ mb: 1, fontWeight: 500 }}>
                Odaberite lokaciju na mapi (kliknite da postavite marker)
              </Typography>
              <Box
                sx={{
                  height: 300,
            ...
            {!newFieldLat && (
              <Alert severity="info">
                Kliknite bilo gde na mapi da odaberete lokaciju terena
              </Alert>
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={!newFieldLat || !newFieldLng}
          >
            Dodaj Teren
          </Button>
        </DialogActions>
