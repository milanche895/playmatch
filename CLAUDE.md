...
The app is a single Next.js process at the repo root.
...
- Firebase Admin SDK used for FCM fallback — credentials stored in `config/` (⚠️ see Known Issues)
...
1. **Firebase credentials in git** (`config/playmatch-*-firebase-adminsdk-*.json`)
